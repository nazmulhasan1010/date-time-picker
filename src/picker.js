    ...
    picker: function (options = {}) {
        ...
        $(fieldClasses).click(function () {
            let pickerActiveClass = $(this).attr('class').split(/\s+/),
                ...
                type = $(this).data('type') === 'date' || $(this).data('type') === 'time' ? $(this).data('type') : typeOp === 'date' || typeOp === 'time' ? $(this).data('type') : 'date',
            ...
            if (type === 'date') {
                if (open === false) {
                    ...
                    function calenderShowing(date, year, month, day, mxLast = null, mnLast = null, start = null) {
                        ...
                        if (rangeA.length > 0 && Number(rangeA[0].month) - 1 === month && rangeA[0].year === year)
                            $(dayDate).children(`span:nth-child(${(Number(rangeA[0].date) + (dayNumber - 1))})`).addClass('nh-span-active');
                        ...
                        $(dayDate).children('span.nh-current').click(function () {
                            ...
                            if (dateRange === true) {
                                ...
                                if (rangeA.length === 0) {
                                    ...
                                    rangeA.push(dateDataA);
                                } else {
                                    $(exception).val(dateFormating(rangeA[0].date, rangeA[0].month, rangeA[0].year) + '-' + dateFormating(selectedDate, selectedMonth, year))
                                    console.log(dateFormating(rangeA[0].date, rangeA[0].month, rangeA[0].year))
                                    console.log(dateFormating(selectedDate, selectedMonth, year))
                                    close();
                                }
                            } else {
